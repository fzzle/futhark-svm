...
entry train [n][m] (X: [n][m]f32) (Y: [n]u8) (k_id: i32)
  ...
  -- TODO: Weights, Cp / Cn
  let (A_I, out, _) =
    loop (A_I, out, p) = ([], out, 0) for i < t do
      loop (A_I, out, p) = (A_I, out, p) for j in i + 1..<t do
        let (si, ci) = (starts[i], counts[i])
        let (sj, cj) = (starts[j], counts[j])
        let c = ci + cj
        let X_p = concat_to c X[si:si + ci] X[sj:sj + cj]
        let (I_p, Y_p) = unzip (map (\i ->
          if i < ci
          then (si + i, 1)
          else (sj + i - ci, -1)) (iota c))
        let (A_p, obj, rho, i) = solve X_p Y_p k C C gamma
          coef0 degree eps max_iter
        -- Only keep non-zero alphas.
        -- Should use a tiny threshold mby
        let A_I_p = filter ((.0) >-> (!=0)) (zip A_p I_p)
        let out[p] = (length A_I_p, obj, rho, i)
        in (A_I ++ A_I_p, out, p + 1)
  let (A, I) = unzip A_I
  ...
  let trues = map (\_ -> true) I -- Would use replicate, but sizes
