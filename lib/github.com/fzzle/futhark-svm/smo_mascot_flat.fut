...
entry solve [n_samples][n_features][n_as]
  ...
  let (_, k, _, A) = loop (b, k, F, A) = (true, 0, F, A) while b do
    ...
    let t0 = segmented_reduce (\a b -> if a.0 < b.0 then a else b) (f32.inf, -1, -1) fs n_s F_u_I
    let (F_u, U, U_s) = unzip3 t0

    let F_l_B = map2 (\y a -> (y && a > 0) || (!y && a < C)) P A
    let F_l = map2 (\b f -> if b then f else -f32.inf) F_l_B F
    let F_max = segmented_reduce (\a b -> if a > b then a else b) (-f32.inf) fs n_s F_l
    ...
    let t2 = segmented_reduce (\a b -> if a.0 > b.0 then a else b) (-f32.inf, -1, -1) fs n_s V_l_I
