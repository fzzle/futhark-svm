...
let scanr 't [n] (op: t -> t -> t) (ne: t) (xs: [n]t): []t =
  reverse <| scan (flip op) ne (reverse xs)

let segmented_scanr [n] 't (op: t -> t -> t) (ne: t)
  (fs: [n]bool) (as: [n]t): [n]t =
  reverse (unzip (scan (\(f0, a) (f1, b) ->
      (f1 || f0, if f0 then a else b `op` a))
    (false, ne) (zip fs as))).1

-- | Distribute a list of f32's `vs` over their respective 
-- areas as indicated by the flags `fs`.
let f32_distribute [n][m] (fs: [n]bool) (vs: [m]f32): [n]f32 =
...
let f32_distribute_endings [n] (fs: [n]bool) (vs: [n]f32): [n]f32 =
  ...
  let masked = map2 (\f v -> if f then v else 0) ends vs
  in segmented_scanr (+) 0 ends masked
